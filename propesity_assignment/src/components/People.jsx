...
const People = () => {
  ...
  return (
    <div style={containerStyles}>
      <h2 style={headingStyles}>People</h2>
      <button onClick={toggleViewMode} style={buttonStyles}>
        {viewMode === 'list' ? 'Switch to Grid View' : 'Switch to List View'}
      </button>
      {viewMode === 'list' ? (
        <ul style={listStyles} className="people-list">
          {people.map((person) => (
            <li key={person.url} style={itemStyles}>
              <img
        ...
        <div style={listStyles} className="people-grid">
          {people.map((person) => (
            <div key={person.url} style={itemStyles}>
              <img
                src={`https://picsum.photos/200/300?random=${Math.random()}`} // Random placeholder image URL from https://picsum.photos/
                alt={person.name}
                style={imageStyles}
